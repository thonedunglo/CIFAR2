    ...
    int batch, int in_c, int out_c, int height, int width) 
{
    ...
    // Grid-stride loop
    for (int i = t_idx + tile_idx * stride; i < total_pixels; i += stride * gridDim.x) {
    ...
    // Atomic update global memory (chỉ 1 lần mỗi thread)
    // Chỉ thread đầu tiên của block IC=0 mới cộng Bias để tránh conflict thừa
    if (ic == 0 && blockIdx.y == 0 && t_idx == 0) {
         atomicAdd(&grad_bias[oc], db); // Code đơn giản hóa: 1 thread cộng bias
         // Lưu ý: Để chính xác tuyệt đối, cần reduce db trong block trước.
         // Nhưng với size ảnh nhỏ, sai số float atomic là chấp nhận được ở phase này.
    }
    // Sửa lỗi logic bias ở trên: Mỗi thread đều tính db riêng, nên tất cả phải atomicAdd.
    // Dòng if trên chỉ đúng nếu ta reduce. Để an toàn nhất (nhưng vẫn nhanh hơn naive):
    if (ic == 0 && tile_idx == 0) { // Hack nhẹ: để tránh bias cộng quá nhiều lần nếu grid X lớn
        // Tốt nhất là atomicAdd luôn:
        atomicAdd(&grad_bias[oc], db);
    } 
    // *Hiệu chỉnh lại logic bias*: Bias không phụ thuộc Input Channel (IC). 
    // Nếu để tất cả block IC cộng vào bias -> Sai (nhân lên IC lần).
    // Chỉ cho các block thuộc IC=0 cộng vào bias.
    
    for (int kh = 0; kh < 3; ++kh) {
        for (int kw = 0; kw < 3; ++kw) {
            int w_idx = ((oc * in_c + ic) * 3 + kh) * 3 + kw;
            atomicAdd(&grad_weight[w_idx], dw[kh][kw]);
        }
    }
}
...
void conv2d_backward_naive(const float* input, const float* grad_out,
    ...
    // 2. Launch Kernel tính Gradient Weight & Bias
    // Chia nhỏ công việc để tăng parallelism
    int num_tiles = 4; 
    dim3 grid_filter(num_tiles, in_c, out_c);
    dim3 block_filter(256); 
    
    // Lưu ý: Kernel filter dùng atomicAdd cộng dồn vào grad_weight/bias.
    // Hàm zero_grads() đã được gọi ở đầu batch trong train_gpu.cu nên không cần memset ở đây.
